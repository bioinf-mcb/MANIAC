...
for file in "${snakemake_params[CHUNK]}"*; do Rscript ${snakemake_params[SORTPATH]} $file ${snakemake_params[SORTED]}$(basename $file); done

sort --parallel=${snakemake_params[MMSEQS_THREADS]} -m -k1,1 -k2,2 -k7,7g "${snakemake_params[SORTED]}"* > "${snakemake_params[MERGED]}"

rm "${snakemake_params[CHUNK]}"* "${snakemake_params[SORTED]}"*

datamash -g 1,2 first 3 first 4 first 5 first 6 first 7 first 8 < "${snakemake_params[MERGED]}" > "${snakemake_output[0]}"
